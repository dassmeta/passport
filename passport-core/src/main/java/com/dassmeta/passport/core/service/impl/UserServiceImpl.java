...
public class UserServiceImpl implements UserService {
	...
	public List<UrpPermission> findUserPermission(String userID) {
		List<UrpPermission> list = this.baseDao.findByNamedQuery("select_permission_by_userid", new Object[] { userID, userID });
		return list;
	}
	...
	public List<UrpPermission> findUserPermission(Long userID, String appCode) {
		...
		if (app != null) {
			...
			list = this.baseDao.findByNamedQuery("select_permission_by_userid_app", new Object[] { userID, appId, userID, appId });
		}
		return list;
	}
	...
	public void enabledUser(AuUserDetail entity) {
		...
		if (aud != null) {
			this.baseDao.executeHQL("update AuUser u set u.deleted=?,u.modifyTime=? where u.id=?").setString(0, "N").setDate(1, new Date()).setSerializable(2, aud.getUserId()).executeUpdate();
			this.baseDao.executeHQL("update AuUserDetail u set u.deleted=?,u.modifyTime=? where u.id=?").setString(0, "N").setDate(1, new Date()).setSerializable(2, aud.getId()).executeUpdate();
		}
	}
	...
	public void disabledUser(AuUserDetail entity) {
		AuUserDetail aud = (AuUserDetail) this.baseDao.get(AuUserDetail.class, entity.getId());
		if (aud != null) {
			this.baseDao.executeHQL("update AuUser u set u.deleted=?,u.modifyTime=? where u.id=?").setString(0, "Y").setDate(1, new Date()).setSerializable(2, aud.getUserId()).executeUpdate();
			this.baseDao.executeHQL("update AuUserDetail u set u.deleted=?,u.modifyTime=? where u.id=?").setString(0, "Y").setDate(1, new Date()).setSerializable(2, aud.getId()).executeUpdate();
		}
	}
