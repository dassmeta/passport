...
import cn.widewisdom.entity.report.AuUserBean;
...
import cn.widewisdom.entity.report.UserDetail;
import cn.widewisdom.entity.urp.UrpPermission;
import cn.widewisdom.entity.urp.UrpRole;
import cn.widewisdom.entity.urp.UrpUserPermission;
import cn.widewisdom.entity.urp.UrpUserRole;
import cn.widewisdom.service.ReportService;
import cn.widewisdom.service.ws.DisabledClient;
...
public class ReportServiceImpl implements ReportService {
	...
	public Page<ReportUserDetail> findForPage(Map<String, Object> params, int page) {
		...
		if ((ascendName != null) && (ascending != null)) {
			o = CriterionBuilder.getOrder(String.valueOf(ascendName), Boolean.valueOf(String.valueOf(ascending)).booleanValue());
		}
		Page<ReportUserDetail> p = this.baseDao.findForPage(ReportUserDetail.class, cri, Integer.valueOf(page), Integer.valueOf(15), o);
		...
		for (ReportUserDetail l : list) {
			...
			if (i == 2) {
				...
				areaSql = "select (select o.org_name from AU_ORG_INFO o where o.org_code=substr('" + l.getOrgCode() + "',1,4))||t.org_name from AU_ORG_INFO t where t.org_code='" + l.getOrgCode()
						+ "'";
			} else if (i == 9) {
				areaSql = "select (select o.org_name from AU_ORG_INFO o where o.org_code=substr('" + l.getOrgCode() + "',1,6))||t.org_name from AU_ORG_INFO t where t.org_code='" + l.getOrgCode()
						+ "'";
			} else if (i == 12) {
				areaSql = "select (select o.org_name from AU_ORG_INFO o where o.org_code=substr('" + l.getOrgCode() + "',1,9))||t.org_name from AU_ORG_INFO t where t.org_code='" + l.getOrgCode()
						+ "'";
			}
	...
	public Page<UserDetail> findForEditedPage(Map<String, Object> params, int page) {
		...
		Page<UserDetail> p = this.baseDao.findForPage(UserDetail.class, cri, Integer.valueOf(page), Integer.valueOf(15), o);
	...
	public void saveUser(ReportUserDetail reportUser) {
		...
		if ("1".equals(reportUser.getIsAdmin())) {
			UrpUserPermission p = new UrpUserPermission();
			UrpPermission up = (UrpPermission) this.baseDao.getByUnique(UrpPermission.class, "permissionKey", "user-report-manage");
			p.setUserId(new Long(userid));
			p.setPermissionId(up.getPid());
			p.setCreateTime(DateFormatUtil.getCurrentSQLDate());
			p.setModifyTime(DateFormatUtil.getCurrentSQLDate());
	...
	public String getAuditedUserRoles(String userId) {
		String sql = "select o.role_name,(select e.app_name from app_app_info e where e.id=o.app_id)as syst from urp_user_role t,urp_role o where t.user_id='" + userId + "'";
	...
	public void saveEdit(String userId, List list1, List list2, List list3, List list4) {
		...
		while (it.hasNext()) {
			s = it.next();
			UrpPermission up = (UrpPermission) this.baseDao.getByUnique(UrpPermission.class, "permissionKey", s.toString().trim());
			UrpUserPermission p = new UrpUserPermission();
			p.setUserId(new Long(userId));
			p.setPermissionId(up.getPid());
			p.setCreateTime(DateFormatUtil.getCurrentSQLDate());
		...
		while (it.hasNext()) {
			...
			p.setUserId(new Long(userId));
			p.setPermissionId(up.getPid());
			this.baseDao.executeSQL("delete URP_USER_PERMISSION t where t.USER_ID='" + userId + "' and t.PERMISSION_ID='" + up.getPid() + "'").executeUpdate();
		}
		...
		while (it.hasNext()) {
			...
			ur.setUserId(new Long(userId));
			ur.setRoleId(new Long(s.toString().trim()));
			this.baseDao.executeSQL("delete URP_USER_ROLE t where t.USER_ID='" + userId + "' and t.ROLE_ID='" + s.toString().trim() + "'").executeUpdate();
		}
	}
